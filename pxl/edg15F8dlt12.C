...
// dlt and taus are correlated
// try to find dlt such that one set of taus works for all Vbias
// dlt = 12 is worse than dlt  =11 at 500V
...
void edg15F8( double Vbias = 300 ) */
{
  ...
  double dlt = 0.0012 * Vbias/250; // [cm] for 250 V
  double N0 = 1127 * 250/Vbias; // [1/um^3] adjust at 250V, scaled 1/dlt^2 at fixed Vbias
  ...
  //double taue = 3.3/F; // at 250V: mdy 3.62
  //double taue = 3.2/F; // at 250V: mdy 3.15
  //double taue = 3.1/F; // at 250V: mdy 2.68
  //double taue = 3.0/F; // at 250V: mdy 2.26
  //double taue = 2.9/F; // at 250V: mdy 2.03
  double taue = 2.8/F; // at 250V: mdy 1.86, 1.83
  //double taue = 2.7/F; // at 250V: mdy 1.99

  if( Vbias > 470 )
    //taue = 2.7/F; // slope at 500V: mdy 5.22 too wide
    //taue = 2.5/F; // slope at 500V: mdy 4.39, 3.81
    //taue = 2.3/F; // slope at 500V: mdy 3.36, 2.74
    //taue = 2.2/F; // slope at 500V: mdy 2.13
    taue = 2.1/F; // slope at 500V: mdy 1.826  (dlt11 reached 1.46)
  //taue = 2.0/F; // slope at 500V: mdy 2.06, 1.826

  // hole trapping: induces anti-charge

  //double tauh = 2.6/F; // [ns] at 250 V mdy 4.73 too wide
  //double tauh = 2.8/F; // [ns] at 250 V mdy 4.46
  //double tauh = 3.2/F; // [ns] at 250 V mdy 4.00
  //double tauh = 3.6/F; // [ns] at 250 V mdy 3.62, 1.86
  double tauh = 3.4/F; // [ns] at 250 V mdy 1.83
  //double tauh = 3.2/F; // [ns] at 250 V mdy 1.86

  if( Vbias > 470 )
    //tauh = 3.0/F; // [ns] for shape at 500V: mdy 4.09 too wide
    //tauh = 2.6/F; // [ns] for shape at 500V: mdy 3.81, 3.36
    //tauh = 2.2/F; // [ns] for shape at 500V: mdy 2.74
    //tauh = 1.8/F; // [ns] for shape at 500V: mdy 2.89
    //tauh = 1.6/F; // [ns] for shape at 500V: mdy 1.87
    tauh = 1.5/F; // [ns] for shape at 500V: mdy 1.826
  //tauh = 1.4/F; // [ns] for shape at 500V: mdy 1.826
  ...
  f3->SetParameter( 0, 0.6*pow( Vbias/500, 3 ) ); // [1/um^3] mdy 1.46 at 500 V
  ...
  for( double z = 0.5; z < pz; z += 1 ) {
    ...
    //double y = int(0.5*pz)-2 - z; // like data, mdy 3.37 at 250V
  ...
  //Double_t smr = 0.011; // [mm] mdy 1.30, 2.34 at 250V
  Double_t smr = 0.010; // [mm] mdy 1.37, 2.30 at 250V
