...

{
  ...
  c1.Print( "s6-150.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33590.root" );
  //TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33807.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "6.6 #upoint10^{15 }p/cm^{2} edge-on 150 V" );
  colpvsy->SetLineColor(1);
  colpvsy->GetXaxis()->SetRangeUser(-0.1,0.1);
  colpvsy->Draw();
  lgnd->AddEntry( colpvsy, " Data 150 V ", "l" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F6_dlt9_e6_h14_V150.root" ); // mean dy 2.46

  TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e11_h28_V150.root" ); // mean dy 1.21
  ...
  c1.Print( "s6-150.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s6-150.ps" );
  ierr = system( "rm -f  s6-150.ps" );
  cout << "evince s6-150.pdf" << endl;
