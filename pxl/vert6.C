...
// model for p irrad phase II sensors, vertical incidence
// 3x3 px

// root -l vert6.C // 6.6E15p/cm2
...
*/
{
  ...
  double N0 =  900 * 250/Vbias; // [1/um^3] at 800V
  ...
  TFile * fn = new
    TFile( Form( "vert6_pz%d_dlt%d_V%d.root",
		 int( pz+0.2 ),
  ...
  for( double F = 0; F < 21.25; F += 0.5 ) {// [E15 p/cm2] fluence
    ...
    for( int iev = 0; iev < nev; ++iev ) {
      ...
      q /= ndiv; // norm to 1
      CvsF->Fill( F, q );
      QvsF->Fill( F, q*11.3 ); // ke in 150 um
      sumq += q;
