...

{
  ...
  c1.Print( "s8-200.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scopedf34435.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "8 #upoint10^{15 }n/cm^{2} edge-on 200 V" );
  ...
  colpvsy->Draw();
  lgnd->AddEntry( colpvsy, " Data 200 V ", "l" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F8_lam10_e4_h4_V200.root" );
  TFile * fsim = TFile::Open( "edg3_px50_F8_dlt11_e4_h4_V200.root" ); // mdy 2.44

  //TFile * fsim = TFile::Open( "edg15_px25_F8_dlt11_e3_h4_V200.root" ); // mdy 1.02
  ...
  c1.Print( "s8-200.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s8-200.ps" );
  ierr = system( "rm -f  s8-200.ps" );
  cout << "evince s8-200.pdf" << endl;
