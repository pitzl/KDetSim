...

{
  ...
  c1.Print( "s2-400.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/shallow28610.root");

  cmsqyvsd->SetStats(0);
  cmsqyvsd->SetTitle( "1.9 #upoint10^{15 }p/cm^{2} shallow 400 V" );
  cmsqyvsd->SetLineColor(1);
  cmsqyvsd->SetLineWidth(3);
  cmsqyvsd->GetXaxis()->SetRangeUser(-0.18,0.18);
  cmsqyvsd->Draw();
  lgnd->AddEntry( cmsqyvsd, " Data 400 V ", "l" );
  int i0 = cmsqyvsd->FindBin(0.095);
  ...
  TFile * fsim = TFile::Open( "edg9_px450_F2_dlt11_e10_h28_V400.root" ); // mdy 

  svsy->SetStats(0);
  svsy->SetLineWidth(2);
  svsy->SetLineColor(617); // Magenta
  svsy->SetMarkerColor(617);
  int k0 = svsy->FindBin(0.095);
  ...
  c1.Print( "s2-400.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s2-400.ps" );
  ierr = system( "rm -f  s2-400.ps" );
  cout << "evince s2-400.pdf" << endl;
