...

{
  ...
  c1.Print( "s8-300.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scopedf34433.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "8 #upoint10^{15 }n/cm^{2} edge-on 300 V" );
  ...
  colpvsy->Draw();
  lgnd->AddEntry( colpvsy, " Data 300 V ", "l" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F8_lam10_e4_h4_V300.root" );
  //TFile * fsim = TFile::Open( "edg3_px50_F8_dlt11_e4_h4_V300.root" ); // mdy 4.75 too wide
  TFile * fsim = TFile::Open( "edg3_px50_F8_dlt10_e4_h4_V300.root" ); // mdy 2.10

  //TFile * fsim = TFile::Open( "edg15_px25_F8_dlt11_e2_h3_V300.root" ); // mdy 1.18
  ...
  c1.Print( "s8-300.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s8-300.ps" );
  ierr = system( "rm -f  s8-300.ps" );
  cout << "evince s8-300.pdf" << endl;
