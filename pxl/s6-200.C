...

{
  ...
  c1.Print( "s6-200.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33589.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "6.6 #upoint10^{15 }p/cm^{2} edge-on 200 V" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F6_dlt9_e6_h14_V200.root" ); // mean dy 1.28

  TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e11_h28_V200.root" ); // mean dy 1.06
  ...
  c1.Print( "s6-200.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s6-200.ps" );
  ierr = system( "rm -f  s6-200.ps" );
  cout << "evince s6-200.pdf" << endl;
