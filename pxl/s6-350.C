...

{
  ...
  c1.Print( "s6-350.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33586.root" ); // 174
  //TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33803.root" ); // 193
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "6.6 #upoint10^{15 }p/cm^{2} edge-on 350 V" );
  colpvsy->SetLineColor(1);
  colpvsy->GetXaxis()->SetRangeUser(-0.1,0.1);
  colpvsy->Draw();
  lgnd->AddEntry( colpvsy, " Data 350 V ", "l" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F6_dlt9_e6_h14_V350.root" ); // mean dy 2.54

  TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e11_h28_V350.root" ); // mean dy
  ...
  c1.Print( "s6-350.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s6-350.ps" );
  ierr = system( "rm -f  s6-350.ps" );
  cout << "evince s6-350.pdf" << endl;
