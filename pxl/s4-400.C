...

{
  ...
  c1.Print( "s4-400.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scopedf34445.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "4 #upoint10^{15 }n/cm^{2} edge-on 400 V" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F4_lam12_e4_h4_V400.root" ); // too wide

  TFile * fsim = TFile::Open( "edg3_px50_F4_lam11_e4_h4_V400.root" ); // good
  ...
  c1.Print( "s4-400.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s4-400.ps" );
  ierr = system( "rm -f  s4-400.ps" );
  cout << "evince s4-400.pdf" << endl;
