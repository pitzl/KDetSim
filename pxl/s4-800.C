...

{
  ...
  c1.Print( "s4-800.ps[", "Portrait" ); // [ opens file
  ...
  TLegend * lgnd = new TLegend( 0.70, 0.70, 0.95, 0.9 );

  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scopedf34453.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "4 #upoint10^{15 }n/cm^{2} edge-on 800 V" );
  ...
  TFile * fsim = TFile::Open( "edg3_px50_F4_lam10_e5_h6_V800.root" ); // good
  //TFile * fsim = TFile::Open( "edg3_px50_F4_lam10_e5_h4_V800.root" ); // worse
  //TFile * fsim = TFile::Open( "edg3_px50_F4_lam10_e5_h8_V800.root" ); // 
  ...
  c1.Print( "s4-800.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s4-800.ps" );
  ierr = system( "rm -f  s4-800.ps" );
  cout << "evince s4-800.pdf" << endl;
