...

{
  ...
  c1.Print( "s6-400.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33585.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "6.6 #upoint10^{15 }p/cm^{2} edge-on 400 V" );
  ...
  TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e11_h28_V400.root" ); // mob4, mdy 1.93
  //TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e8_h28_V400.root" ); // mob3, mdy 2.16
  ...
  c1.Print( "s6-400.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s6-400.ps" );
  ierr = system( "rm -f  s6-400.ps" );
  cout << "evince s6-400.pdf" << endl;
