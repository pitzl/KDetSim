...

{
  ...
  c1.Print( "s2-500.ps[", "Portrait" ); // [ opens file
  ...
  TLegend * lgnd = new TLegend( 0.45, 0.20, 0.70, 0.35 );

  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/shallow28602.root");

  cmsqyvsd->SetStats(0);
  cmsqyvsd->SetTitle( "1.9 #upoint10^{15 }p/cm^{2} shallow 500 V" );
  cmsqyvsd->SetLineColor(1);
  cmsqyvsd->SetLineWidth(3);
  cmsqyvsd->GetXaxis()->SetRangeUser(-0.18,0.18);
  cmsqyvsd->Draw();
  lgnd->AddEntry( cmsqyvsd, " Data 500 V ", "l" );
  ...
  //TFile * fsim = TFile::Open( "edg9_px450_F2_dlt15_e9_h28_V500.root" ); // mdy 1.05
  TFile * fsim = TFile::Open( "edg9_px450_F2_dlt11_e10_h28_V500.root" ); // mdy 

  svsy->SetStats(0);
  svsy->SetLineWidth(2);
  svsy->SetLineColor(617); // Magenta
  svsy->SetMarkerColor(617);
  int k0 = svsy->FindBin(0.0875);
  ...
  c1.Print( "s2-500.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s2-500.ps" );
  ierr = system( "rm -f  s2-500.ps" );
  cout << "evince s2-500.pdf" << endl;
