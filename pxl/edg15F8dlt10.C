...
void edg15F8( double Vbias = 300 ) */
{
  ...
  //double Vbias = 800;
  //double Vbias = 600;
  double Vbias = 500; // full depletion at 8E15 n/cm2, too wide
  //double Vbias = 400;
  //double Vbias = 300;
  ...
  double N0 = 1620 * 250/Vbias; // [1/um^3] adjust at 250V, scaled 1/dlt^2 at fixed Vbias
  ...
  double taue = 4.4/F; // [ns] at 250V: mdy 1.83

  if( Vbias > 370 )
    taue = 3.4/F; // [ns] from slope at 400 V mdy 1.99

  if( Vbias > 470 )
    taue = 3.2/F; // [ns] from slope at 500 V, mean dy 2.59

  if( Vbias > 770 )
    taue = 3.0/F; // [ns] from slope at 800 V, mean dy 1.30

  // hole trapping: induces anti-charge

  double tauh = 3.2/F; // [ns] at 250 V: mdy 1.83
