...

{
  ...
  c1.Print( "s6-250.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33588.root" ); // 174
  //TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33805.root" ); // 193
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "6.6 #upoint10^{15 }p/cm^{2} edge-on 250 V" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F6_dlt9_e6_h14_V250.root" ); // mean dy 2.55

  TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e11_h28_V250.root" ); // mean dy 1.86
  ...
  c1.Print( "s6-250.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s6-250.ps" );
  ierr = system( "rm -f  s6-250.ps" );
  cout << "evince s6-250.pdf" << endl;
