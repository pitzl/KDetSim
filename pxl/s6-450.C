...

{
  ...
  c1.Print( "s6-450.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33584.root" );
  //TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33801.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "6.6 #upoint10^{15 }p/cm^{2} edge-on 450 V" );
  colpvsy->SetLineColor(1);
  colpvsy->GetXaxis()->SetRangeUser(-0.1,0.1);
  colpvsy->Draw();
  lgnd->AddEntry( colpvsy, " Data 450 V ", "l" );
  ...
  TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e11_h28_V450.root" ); // mob4, mean dy 2.00
  //TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e8_h28_V450.root" ); // mob3, mean dy 2.11
  ...
  c1.Print( "s6-450.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s6-450.ps" );
  ierr = system( "rm -f  s6-450.ps" );
  cout << "evince s6-450.pdf" << endl;
