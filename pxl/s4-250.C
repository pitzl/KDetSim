...

{
  ...
  c1.Print( "s4-250.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scopedf34447.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "4 #upoint10^{15 }n/cm^{2} edge-on 250 V" );
  ...
  TFile * fsim = TFile::Open( "edg3_px50_F4_lam12_e4_h4_V250.root" ); // 
  ...
  c1.Print( "s4-250.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s4-250.ps" );
  ierr = system( "rm -f  s4-250.ps" );
  cout << "evince s4-250.pdf" << endl;
