...

{
  ...
  c1.Print( "s4-300.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scopedf34446.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "4 #upoint10^{15 }n/cm^{2} edge-on 300 V" );
  ...
  TFile * fsim = TFile::Open( "edg3_px50_F4_lam12_e4_h4_V300.root" );
  ...
  c1.Print( "s4-300.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s4-300.ps" );
  ierr = system( "rm -f  s4-300.ps" );
  cout << "evince s4-300.pdf" << endl;
