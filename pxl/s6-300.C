...

{
  ...
  c1.Print( "s6-300.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scoped33587.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "6.6 #upoint10^{15 }p/cm^{2} edge-on 300 V" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F6_dlt9_e6_h14_V300.root" ); // mean dy 2.03

  TFile * fsim = TFile::Open( "edg9_px50_F6_dlt9_e11_h28_V300.root" ); // mean dy 1.71
  ...
  c1.Print( "s6-300.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s6-300.ps" );
  ierr = system( "rm -f  s6-300.ps" );
  cout << "evince s6-300.pdf" << endl;
