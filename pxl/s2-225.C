...

{
  ...
  c1.Print( "s2-225.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/shallow28607.root");

  cmsqyvsd->SetStats(0);
  cmsqyvsd->SetTitle( "1.9 #upoint10^{15 }p/cm^{2} shallow 225 V" );
  cmsqyvsd->SetLineColor(1);
  cmsqyvsd->GetXaxis()->SetRangeUser(-0.18,0.18);
  cmsqyvsd->Draw();
  lgnd->AddEntry( cmsqyvsd, " Data 225 V ", "l" );
  ...
  TFile * fsim = TFile::Open( "edg9_px450_F2_dlt11_e10_h28_V225.root" );
  ...
  c1.Print( "s2-225.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s2-225.ps" );
  ierr = system( "rm -f  s2-225.ps" );
  cout << "evince s2-225.pdf" << endl;
