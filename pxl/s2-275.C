...

{
  ...
  c1.Print( "s2-275.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/shallow28606.root");

  cmsqyvsd->SetStats(0);
  cmsqyvsd->SetTitle( "1.9 #upoint10^{15 }p/cm^{2} shallow 275 V" );
  cmsqyvsd->SetLineColor(1);
  cmsqyvsd->GetXaxis()->SetRangeUser(-0.18,0.18);
  cmsqyvsd->Draw();
  lgnd->AddEntry( cmsqyvsd, " Data 275 V ", "l" );
  ...
  TFile * fsim = TFile::Open( "edg9_px450_F2_dlt11_e10_h28_V275.root" );
  ...
  c1.Print( "s2-275.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s2-275.ps" );
  ierr = system( "rm -f  s2-275.ps" );
  cout << "evince s2-275.pdf" << endl;
