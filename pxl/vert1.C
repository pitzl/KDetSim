...
*/
{
  ...
  KPixel * det = new KPixel( 1, 1*px, 1*py, pz ); // pixel, [um]

  det->SetUpVolume( 2.0, 2.0, 1.0 ); // um cubes (speed vs granularity)

  //                  center          half-implant    [um]  pot
  det->SetUpPixel( 0, 0.5*px, 0.5*py, 0.45*px, 0.45*py, 0.5, 16385 ); // collecting electrode at Grnd
  ...
  TFile * fn = new
    TFile( Form( "vert1_dlt%d_V%d.root",
  ...
  TH1F * field = det->Draw1D( "EZxz1", 0.5*py, 2, 0.5*px );
  ...
  for( int ii = 1; ii < field->GetNbinsX(); ++ii ) {
    ...
    double N = f3->Eval( 0.5*px, 0.5*py, z );
  ...
  TH1F * wpot = det->Draw1D( "WPxz1", 0.5*py, 2, 0.5*px );
  wpot->Draw();  
  cWP1.Update();
