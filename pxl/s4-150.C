...

{
  ...
  c1.Print( "s4-150.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scopedf34450.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "4 #upoint10^{15 }n/cm^{2} edge-on 150 V" );
  ...
  //TFile * fsim = TFile::Open( "edg3_px50_F4_lam16_e4_h4_V150.root" ); // a little wide
  TFile * fsim = TFile::Open( "edg3_px50_F4_lam15_e4_h4_V150.root" ); // better
  ...
  c1.Print( "s4-150.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s4-150.ps" );
  ierr = system( "rm -f  s4-150.ps" );
  cout << "evince s4-150.pdf" << endl;
