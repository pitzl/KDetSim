...

{
  ...
  c1.Print( "s2-600.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/shallow28601.root");

  cmsqyvsd->SetStats(0);
  cmsqyvsd->SetTitle( "1.9 #upoint10^{15 }p/cm^{2} shallow 600 V" );
  cmsqyvsd->SetLineColor(1);
  cmsqyvsd->SetLineWidth(3);
  cmsqyvsd->GetXaxis()->SetRangeUser( -0.18, 0.18 );
  cmsqyvsd->Draw();
  lgnd->AddEntry( cmsqyvsd, " Data 600 V ", "l" );
  ...
  TFile * fsim = TFile::Open( "edg9_px450_F2_dlt11_e10_h28_V600.root" ); // mdy 
  ...
  c1.Print( "s2-600.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s2-600.ps" );
  ierr = system( "rm -f  s2-600.ps" );
  cout << "evince s2-600.pdf" << endl;
