...

{
  ...
  c1.Print( "s4-500.ps[", "Portrait" ); // [ opens file
  ...
  TFile * fdata = TFile::Open( "/home/pitzl/eudaq/tele-scope/scopedf34444.root" );
  colpvsy->SetStats(0);
  colpvsy->SetTitle( "4 #upoint10^{15 }n/cm^{2} edge-on 500 V" );
  ...
  TFile * fsim = TFile::Open( "edg3_px50_F4_lam10_e4_h4_V500.root" );
  ...
  c1.Print( "s4-500.ps]" ); // ] closes file
  int ierr;
  ierr = system( "ps2pdf s4-500.ps" );
  ierr = system( "rm -f  s4-500.ps" );
  cout << "evince s4-500.pdf" << endl;
